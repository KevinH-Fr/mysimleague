module BadgesHelper
  ...
  def positif_negatif_badge(value)
    ...
    if value > 0
      content_tag(:div, class: "fa fa-fade badge border border-success") do 
      ...
      content_tag(:div, class: "fa fa-fade badge border border-danger") do 
  ...
  def badge_solde_parieur(user)
      solde_parieur = somme_paris_user(User.all)[user.id][:sum]
